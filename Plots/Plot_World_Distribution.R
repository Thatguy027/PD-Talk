...
wrld_pt <- ggplot(data = world) +
  base_theme +
  geom_sf(fill = "cornsilk", inherit.aes = T) +
...
wrld_pt + geom_sf(data = sites, size = 2, shape = 25, fill = highlight_color) + 
  base_theme + 
  coord_sf(ylim = c(-50, 90)) +
  theme(plot.background = element_rect(fill = background_color)) 

ggsave(filename = "World_Plot.pdf", height = 10, width = 14)

hawaii + geom_sf(data = sites, size = 2, shape = 25, fill = highlight_color) + coord_sf(crs = st_crs(4135), xlim = c(-161, -154), ylim = c(18, 23), expand = FALSE, datum = NA)

ggsave(filename = "Hawaii.pdf", height = 10, width = 10)
...
euro_pt + geom_sf(data = sites, size = 2, shape = 25, fill = highlight_color) + coord_sf(crs = st_crs(4135), xlim = c(-15, 40), ylim = c(30, 70), expand = FALSE, datum = NA)

ggsave(filename = "Europe.pdf", height = 10, width = 10)