...
manplot_edit <- function(plot_df, bf_line_color = "gray") {
  ...
  plots <- lapply(plot_traits, function(i) {
    plot_df %>%
      ...
      ggplot2::scale_color_manual(values = c("black","#D7263D","#2790F9")) +
      ggplot2::scale_alpha_manual(values = c(0.5,1,1)) +
  ...
  if(nrow(cis) != 0) {
    plot <- plot + 
      ...
      ggplot2::geom_point(data = cis, ggplot2::aes(x=pos/1e6, y=(1.05*maxlod)),
                          fill ="#D7263D", shape=25, size=3.2, show_guide = FALSE) +
                            ...
                            site_of_interest = 11875145) 
{
  ...
  if (!is.null(site_of_interest)) {
    tajimas_d_plot <- tajimas_d_plot + 
      ggplot2::geom_vline(ggplot2::aes(xintercept = site_of_interest/1e+06), color = "#D7263D", alpha = 0.7, size = 1)
  }
...
gene_model <- function(df, 
  ...
  if(strand == "Watson"){
    ...
    #   geom_polygon(aes(x = x, y = y), fill = "#D7263D",color = "black", data = endUTR_pt)+
    #   theme_void()
...
modified_ld_plot <- function (plot_df, trait = NULL) 
{
  ...
  if (ncol(test) == 1) {
    ...
    ldplot <- ggplot2::ggplot(LDs) + ggplot2::aes(x = factor(SNP1, levels = unique(SNP1), ordered = T), 
      ...
      ggplot2::geom_text(ggplot2::aes(label = signif(corr, 3)), fontface = "bold", size = 12, family = number_font) + 
...
plot_bar_phylo <- function(df, pt_colors){
  ...
  df%>%
    ...
    scale_fill_manual(values= pt_colors,name = expression(paste("Variation at ", italic("ben-1"))))+
    theme_bw()+
    labs(x = "Strain", y = paste0("Albendazole Resistance"))+
    coord_flip()+
